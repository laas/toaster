/* 
 * File:   AdaptationHumanReader.cpp
 * Author: sdevin
 * 
...
AdaptationHumanReader::AdaptationHumanReader(ros::NodeHandle& node, std::string topic) {
    std::cout << "[area_manager] Initializing AdaptationHumanReader" << std::endl;
    // Starts listening to the topic
    sub_ = node.subscribe(topic, 1, &AdaptationHumanReader::humanJointStateCallBack, this);
}

void AdaptationHumanReader::humanJointStateCallBack(const toaster_msgs::HumanListStamped::ConstPtr& msg) {
